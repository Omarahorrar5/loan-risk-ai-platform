...
with mlflow.start_run(run_name="MLP_Training_Run"):
    # Log hyperparameters manually for quick reference
    mlflow.log_params({
        "epochs": EPOCHS,
        "batch_size": BATCH_SIZE,
        "learning_rate": LR,
        "threshold": THRESHOLD,
        "network_structure": str(HIDDEN)
    })
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # Log metrics to MLflow for graphs
    ...
    mlflow.log_metric("risky_recall", report["Risky"]["recall"])
    mlflow.log_metric("risky_precision", report["Risky"]["precision"])
